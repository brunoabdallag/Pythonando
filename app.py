...
# Criando modulo de autenticação para integrar a uma rota desejável
...
def obter_postagens(autor):
    ...
    # Inserindo informações do banco na lista
    list_postagens = []
    # Utilizando for para inserir as informações da biblioteca
...
# Obter postagem por ID - Get com ID  - GET http://localhost:5000/postagem/1
...
# Deletar uma postagem existente - DELETE - http://localhost:5000/postagem/1
...
def obter_autores(autor):
    autores = Autor.query.all() # Extrair os dados de uma tabela
    lista_de_autores = []
